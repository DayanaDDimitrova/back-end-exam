...
class MediaUploadConfigurationForm extends ConfigFormBase {
  ...
  public function __construct(
    ...
    foreach ($videoBundles as $item) {
      $this->videoBundles[$item->bundle()] = sprintf('%s', $item->bundle());
    }

    $audioBundles = $this->entityTypeManager
      ->getStorage('media')
      ->loadByProperties(['bundle' => 'audio']);
    foreach ($audioBundles as $item) {
      $this->audioBundles[$item->bundle()] = sprintf('%s', $item->bundle());
    }

    $documentBundles = $this->entityTypeManager
      ->getStorage('media')
      ->loadByProperties(['bundle' => 'document']);
    foreach ($documentBundles as $item) {
      $this->documentBundles[$item->bundle()] = sprintf('%s', $item->bundle());
    }
    ...
    foreach ($imageBundles as $item) {
      $this->imageBundles[$item->bundle()] = sprintf('%s', $item->bundle());
    }
  }
  ...
  public function buildForm(
    ...
    $form['video']['video_bun'] = [
      ...
      '#ajax' => [
        'callback' => [static::class, 'populateFields'],
        'event'    => 'change',
      ],
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $config->save();
    $this->messenger()->addStatus($this->t('Configuration done.'));
  }
  ...
  private function checkMediaBundles(array &$form) {
    foreach ($this->mediaBundle as $item => $value) {
      if (empty($this->entityTypeManager
        ->getStorage('media')
        ->loadByProperties(['bundle' => $item]))
      ) {
        $this->messenger()->addWarning($this->t("You don't seem to have media of the @item type. You should <a href=':link'>create one</a>", [
          '@item' => $item,
          ':link' => Url::fromRoute('entity.media.add_form', ['media_type' => $item])->toString(),
        ]), 'warning');
